...
public class CardTests
{
    ...
    private static IEnumerable<TestCaseData> GetTestCaseDataForGetTrickValue()
    {
        ...
        yield return new TestCaseData(Suit.Hearts, Suit.Hearts, new Card(Suit.Hearts, Rank.King), 513);
        yield return new TestCaseData(Suit.Clubs, Suit.Clubs, new Card(Suit.Clubs, Rank.Ace), 514);
    ...
    [Test]
    public void Clone_ReturnsDistinctCard_WithSameValues()
    {
        // Arrange
        var original = new Card(Suit.Spades, Rank.Ace);
        ...
        Assert.That(clone, Is.InstanceOf<Card>());

        var clonedCard = (Card)clone;
        Assert.That(clonedCard.Suit, Is.EqualTo(original.Suit));
        Assert.That(clonedCard.Rank, Is.EqualTo(original.Rank));
        Assert.That(clonedCard.ShuffleValue, Is.EqualTo(original.ShuffleValue));
    }
